...
- **Devloop** — a throwaway git worktree per task where the agent edits and runs THE REPO'S OWN checks (a linter, a formatter, a test target) before anything is pushed, so a file that does not parse is discovered in the conversation rather than in CI. The checks are argv the operator wrote, keyed by name in `persona.yaml`; the model passes a name, never a command. They run under Seatbelt with no outbound network, no writes outside the worktree, and no reads of the credentials on disk. Two approvals bound it: one to start, one to push.
- **Workspace** — read-only tools over the operator's local repo mirrors (list repos, tree, offset-paged reads, `git grep`), confined to one configured root with `.git` internals unreadable. This is how a review verifies a claim against the actual source instead of guessing forge paths one API read at a time.
...
**Built-in faculties:** memory · schedule · tasks · skills · documents · code · files · delegate · jobs · workspace · devloop
