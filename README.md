...
- **Artifacts** — long structured output (an MR review with findings and diff hunks) publishes as a styled, self-contained page on a hosted URL instead of a wall of chat text; the id in the URL is unguessable and minted by the runtime, publishing rides the approval tap, and the page's comment box lands back in chat as a gated background turn. Republishing the same id updates the page in place, so a staged review keeps one URL as it grows.
...
**Built-in faculties:** memory · schedule · tasks · skills · documents · code · files · delegate · jobs · artifacts
**Service connectors:** Gmail · Google Calendar · Google Drive · Yahoo Mail · ClickUp · Splitwise · budget-tracker · GitLab
