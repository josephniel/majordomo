...
from adapters.chat.telegram import _TelegramReplyStream
...
class FakeBot:
    def __init__(self, fail_edit_after: int | None = None) -> None:
        self.sent: list[str] = []
        self.edits: list[str] = []
    ...
    async def edit_message_text(self, text, chat_id=None, message_id=None):
        if self._fail_edit_after is not None and len(self.edits) >= self._fail_edit_after:
            raise RuntimeError("429 flood control")
...
class TestTelegramReplyStream:
    ...
    async def test_pushes_are_throttled(self):
        """Editing per token earns a 429 that stalls the whole point."""
        bot = FakeBot()
        s = _stream(bot)
        for i in range(40):
            await s.push("z" * (60 + i))
        assert len(bot.sent) + len(bot.edits) == 1, "throttled to the first paint"
    ...
    async def test_a_failed_final_paint_withdraws_rather_than_truncating(self):
        ...
        """
        bot = FakeBot(fail_edit_after=0)
        s = _stream(bot)
        await s.push("a" * 60)          # opens the message
