...
class TestSaveFact:
    ...
    async def test_reference_scope_needs_no_domain_key(self, memory):
        _, entry = await memory.save_fact(
            FactCandidate("reference", "crm-docs repo has the schema")
        )
        assert entry is not None
    ...
    async def test_near_duplicate_rejected_with_guidance(self, memory):
        ...
        msg, entry = await memory.save_fact(
            FactCandidate("user", "The user's dog is named Bantay!")
        )
        assert entry is None
    ...
    async def test_source_recorded_in_metadata(self, memory, memdb, persona_id):
        _, entry = await memory.save_fact(
            FactCandidate("user", "reflected fact", provenance="reflection")
        )
...
class TestAutoRecall:
    ...
    async def test_format_includes_scope_label(self, memory):
        await memory.save_fact(
            FactCandidate("domain", "ClickUp workspace id is 12345", domain_key="clickup")
        )
...
class TestCompaction:
    ...
    async def test_update_triggers_recompaction(self, memory, memdb, persona_id):
        ...
        result = await update.handler(
            {"id": str(entry.id), "content": "the user lives in Makati"}, ToolContext()
        )
...
class TestTools:
    async def test_memory_save_tool(self, memory):
        ...
        result = await save.handler(
            {"scope": "user", "content": "tool-saved fact"},
            ToolContext(),
        )
    ...
    async def test_history_search_tool_uses_chat_context(self, memory, history, persona_id):
        ...
        result = await search.handler(
            {"query": "durian tariffs"},
            ToolContext(chat_id=CHAT_ID),
        )
    ...
    async def test_history_search_absent_without_history(self, memdb, persona_id):
        m = LongTermMemory(
            db=memdb, persona_id=persona_id, summarizer=FakeSummarizer(), history=None
        )
...
class TestLinks:
    async def _two_facts(self, memory):
        ...
        _, b = await memory.save_fact(
            FactCandidate("user", "the homelab runs Proxmox virtualization")
        )
        return a, b
...
class TestPinned:
    async def test_pin_renders_verbatim_with_id(self, memory):
        _, entry = await memory.save_fact(
            FactCandidate("user", "the user's daughter is named Liwayway")
        )
    ...
    async def test_pinned_exempt_from_truncation(self, memory, memdb, persona_id):
        ...
        _, entry = await memory.save_fact(
            FactCandidate("agent", "the assistant must always reply in English")
        )
        pin = tool_by_name(memory, "memory_pin")
...
class TestStaleness:
    async def _backdate(self, memdb, entry_id, days):
        async with memdb._acquire() as conn:
            await conn.execute(
                "UPDATE memory_entries SET verified_at = NOW() - $2::interval WHERE id=$1",
                entry_id,
                timedelta(days=days),
            )

    async def test_stale_volatile_annotated_in_recall(self, memory, memdb, persona_id):
        _, e = await memory.save_fact(
            FactCandidate("agent", "the deploy flag is --prod", volatile=True)
        )
    ...
    async def test_stale_pinned_fact_annotated(self, memory, memdb, persona_id):
        _, e = await memory.save_fact(
            FactCandidate("agent", "credentials live under data/credentials/", volatile=True)
        )
